...
const ADDRESSES = [
  { id: "home",       icon: "home",   label: "Home",       sub: "221B Baker Street, North Wing" },
  { id: "university", icon: "school", label: "University", sub: "Tech Campus, Block C, Room 402" },
];
...
const STANDARDS = [
  { icon: "schedule",   title: "Instant Flex",      desc: "Cancel or pause your meal up to 60 minutes before the window starts." },
  { icon: "restaurant", title: "Hygiene Verified",  desc: "Every kitchen undergoes a weekly 45-point health and safety inspection." },
  { icon: "eco",        title: "Zero Waste",        desc: "Compostable packaging and optimized portions to eliminate food waste." },
];
  ...
  return (
    <div className="bg-white rounded-[2rem] overflow-hidden flex flex-col border border-stone-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        {vendor.badges.length > 0 && (
          <div className="absolute top-4 left-4 flex gap-2">
            {vendor.badges.map((b) => (
              <span key={b.label} className={`${b.cls} px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider`}>
                {b.label}
              </span>
            ))}
          </div>
      ...
      <div className="p-8 flex flex-col flex-1">
        ...
        <p className="text-stone-500 text-sm mb-6 line-clamp-2 leading-relaxed">{vendor.desc}</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 border-y border-stone-100 py-5">
          <div>
            ...
            <p className="text-xl font-black text-stone-900">
              {vendor.price} <span className="text-[10px] text-stone-400 uppercase font-bold">Starting</span>
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-1">Active Now</p>
            <p className="text-xl font-black text-amber-500">
              {vendor.subs} <span className="text-[10px] text-stone-400 uppercase font-bold">Subs</span>
            </p>
          </div>
        </div>
...
function ZoneSeparator({ icon, iconColor, label }) {
  return (
    <div className="flex items-center gap-4 mb-8">
      ...
      <div className={`flex items-center gap-2.5 bg-white px-6 py-2 rounded-full border border-stone-200 shadow-sm`}>
        <span className={`material-symbols-outlined text-sm ${iconColor}`}>{icon}</span>
        <span className="text-xs font-black uppercase tracking-[0.2em] text-stone-700">{label}</span>
      </div>
      <div className="h-px flex-1 bg-gradient-to-r from-transparent via-stone-200 to-transparent" />
    </div>
  );
        ...
        <main className="pt-32 pb-24 px-6 md:px-12 max-w-[1600px] mx-auto">
          ...
          <section className="mb-16">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xs font-black uppercase tracking-[0.3em] text-stone-400">My Delivery Locations</h2>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              {ADDRESSES.map((addr) => {
                ...
                return (
                  ...
                  >
                    <span className="material-symbols-outlined text-xl">{addr.icon}</span>
                    <div className="text-left">
                      <p className={`text-xs font-black uppercase tracking-widest leading-none mb-1 ${isActive ? "text-white" : "text-stone-400"}`}>
                        {addr.label}
                      </p>
                      <p className={`text-sm font-bold ${isActive ? "text-white/70" : "text-stone-500"}`}>
                        {addr.sub}
                      </p>
                    </div>
                  </button>
              ...
              <button className="flex items-center gap-3 px-6 py-4 rounded-2xl border-2 border-dashed border-stone-300 text-stone-400 hover:text-stone-700 hover:border-stone-500 transition-all">
                ...
                <span className="text-sm font-black uppercase tracking-widest">Add New Address</span>
              </button>
            </div>
          </section>
          ...
          <header className="mb-12">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
              <div>
                ...
                <h1 className="text-5xl md:text-6xl font-black tracking-tighter text-stone-900">Daily Meal Plans</h1>
              </div>
          ...
          {DELIVERY_ZONES.map((zone) => (
            <div key={zone.id} className="mb-20">
              <ZoneSeparator icon={zone.icon} iconColor={zone.iconColor} label={zone.label} />
          ...
          <div className="mt-32 grid grid-cols-12 gap-12 pt-24 border-t border-stone-200">
            <div className="col-span-12 lg:col-span-8">
              <h2 className="text-3xl font-black tracking-tighter mb-12 text-stone-900">The AhaarMitra Standard</h2>
